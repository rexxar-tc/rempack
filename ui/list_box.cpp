...
#if 0
#include <rmkit.h>                 // brings in stbtext::setup_font()

namespace stbtext {
    extern bool  did_setup;        // already provided by rmkit
    extern void  setup_font();     // forward‑declared inside rmkit.h
    extern stbtt_fontinfo font;    // **static in this TU**, safe to touch
}
...
struct FontGuard {
    ...
    {
        std::cout<<"AFRAID\n";
        if (stbtext::font.data == nullptr) {      // local copy still blank?
            stbtext::did_setup = false;           // force rmkit’s init path
            std::cout<<"UPSET\n";
            stbtext::setup_font();                // run it *inside this TU*
        }
    }
};

// one instance, constructed at load time
__attribute__((used)) static FontGuard ensure_font_ready;
#endif
...
namespace widgets{
    ...
    shared_ptr<ListBox::ListItem> ListBox::add(const string &label, const any &object) {
